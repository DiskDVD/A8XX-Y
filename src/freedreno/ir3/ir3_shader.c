...
ir3_apply_a8xx_tuning(struct ir3_shader_variant *v)
{
   ...
   if (v->key.enable_ubwc_paths) {
      /* A8xx supports UBWCv6; keep image/SSBO lowering in the fast path. */
      v->shader_options.nir_options.robust_modes =
         v->shader_options.nir_options.robust_modes;
   }

   if (v->key.prefer_fp16_math && ir3_is_a810_or_a825(v->compiler)) {
      /* Prefer native mediump lowering for temporaries when it is legal. */
      v->shader_options.real_wavesize =
         MIN2(v->shader_options.real_wavesize, IR3_SINGLE_OR_DOUBLE);
   }
