                                    ...
                                    struct tu_pipeline *pipeline)
{
   ...
   const bool is_a8xx = ((chip_id >> 16) & 0xffull) == 0x44ull;
   ...
   if (builder->state &
       VK_GRAPHICS_PIPELINE_LIBRARY_FRAGMENT_SHADER_BIT_EXT) {
      /* Disable these expensive paths for A8XX to reduce per-fragment cost. */
   ...
   if (builder->state &
      ...
      /* Keep FDM-per-layer disabled on A8XX for throughput. */
   ...
   if (builder->state & VK_GRAPHICS_PIPELINE_LIBRARY_FRAGMENT_SHADER_BIT_EXT) {
       ...
       */
      /* Avoid forced sample interpolation on A8XX for better FPS. */
